...
#include "lilia/app/domain/analysis/eco_opening_db.hpp"
...
namespace lilia::app::view::ui
{
  ...
  namespace
  {
                     ...
                     const Theme &th)
    {
      ...
      const float y = snapf(center.y - h - kTipArrowH - 4.f);
      ...
      t.setPosition(snap({x + kTipPadX - b.left, y + kTipPadY - b.top}));
      win.draw(t);
    }
    ...
    void drawSlot(sf::RenderTarget &rt,
                  ...
                  bool pressed)
    {
      drawBevelButton(rt, r, th.button, hovered, pressed);
      sf::RectangleShape hair({r.width - 2.f, r.height - 2.f});
      hair.setPosition(snap({r.left + 1.f, r.top + 1.f}));
    ...
    void drawRobot(sf::RenderWindow &win, const sf::FloatRect &slot, sf::Color col)
    {
      ...
      head.setPosition(snap({cx, cy + s * 0.04f}));
    ...
    static std::string buildReplayOpeningTitle(const domain::ReplayInfo &h)
    {
      // If openingName is missing or is accidentally just "B28", resolve via DB.
      return domain::analysis::EcoOpeningDb::resolveOpeningTitle(h.eco, h.openingName);
    }
  ...
  void MoveListView::setPosition(const MousePos &pos)
  {
    m_position = pos;
  }

  void MoveListView::setReplayHeader(std::optional<domain::ReplayInfo> header)
  {
  ...
  std::size_t MoveListView::getMoveIndexAt(const MousePos &pos) const
  {
  ...
  MoveListView::Option MoveListView::getOptionAt(const MousePos &pos) const
  {
    const float localX = pos.x - m_position.x;
    const float localY = pos.y - m_position.y;
  ...
  {
    const Theme &th = m_theme.uiTheme();
    ...
    // --- Panel shadow + border ---
    {
      ...
      drawPanelShadow(window, panelRect);

      sf::RectangleShape border({panelRect.width, panelRect.height});
      border.setPosition(snap({panelRect.left, panelRect.top}));
    ...
    sf::Color headerCol = darken(th.panel, 10);
    ...
    header.setPosition(snap({(m_width - hb.width) * 0.5f - hb.left,
    ...
    if (m_replay_header)
    {
      const domain::ReplayInfo &rh = *m_replay_header;

      const std::string openingTitle = buildReplayOpeningTitle(rh);
      const std::string ecoNorm = domain::analysis::EcoOpeningDb::nameForEco(rh.eco).empty()
                                      ? std::string{}
                                      : ("ECO " + domain::analysis::EcoOpeningDb::resolveOpeningTitle(rh.eco, rh.eco));

      // Opening title (centered)
      std::string openDisp = openingTitle.empty() ? "Unknown Opening" : openingTitle;
      openDisp = ellipsizeMiddle(m_font, kReplayOpeningFontSize, openDisp,
      ...
      openTxt.setPosition(snap({(m_width - ob.width) * 0.5f - ob.left,
                                subY + 10.f - ob.top}));
      window.draw(openTxt);
      ...
      infoBg.setPosition(snap({infoRect.left, infoRect.top}));
      infoBg.setFillColor(darken(th.panel, 4));
      window.draw(infoBg);
      drawBevelFrame(window, infoRect, darken(th.panel, 4), th.panelBorder);
      ...
      for (std::size_t i = 0; i < items.size(); ++i)
      {
        ...
        const std::string v = ellipsizeMiddle(m_font, kReplayMetaFontSize, v0, maxV);

        sf::Text vTxt(v, m_font, kReplayMetaFontSize);
        vTxt.setFillColor(th.text);
        vTxt.setPosition(snap({x + labelW, y}));
    ...
    else
    {
      ...
      sub.setPosition(snap({(m_width - sb.width) * 0.5f - sb.left,
    ...
    if (showCheck)
    {
      ...
      float x = snapf(c.x - mb.width * 0.5f - mb.left);
      float y = snapf(c.y - 6.f - mb.height - mb.top - prog * 20.f);
    ...
    fenTxt.setPosition(snap({textX, kHeaderH + (kFenH - fb.height) * 0.5f - fb.top - 2.f}));
    window.draw(fenTxt);
    ...
    const sf::Color rowEven = lighten(th.panel, 4);
    const sf::Color rowOdd = darken(th.panel, 2);
    const sf::Color hiRow = lighten(th.buttonActive, 6);
    ...
    for (std::size_t i = 0; i < totalLines; ++i)
    {
      ...
      if (i == m_rows.size() && !m_result.empty())
      {
        ...
        res.setPosition(snap({(m_width - rb.width) * 0.5f - rb.left, y}));
        window.draw(res);
        continue;
      }
      ...
      num.setPosition(snap({kPaddingX, y}));
      window.draw(num);
