...
void HistorySystem::reset(const std::string& startFen, int startEval, const TimeView& startTime) {
  m_fen_history.clear();
...
void HistorySystem::onMoveCommitted(const MoveView& mv, const std::string& fenAfter, int evalAfter,
                                    const TimeView& timeAfter) {
  m_move_history.push_back(mv);

  m_fen_history.push_back(fenAfter);
  m_eval_history.push_back(evalAfter);
  ...
  m_eval_cp.store(evalAfter);
  m_view.updateEval(evalAfter);
...
bool HistorySystem::handleMoveListClick(core::MousePos mp, PremoveSystem& premove) {
  ...
  if (idx == kInvalidMoveIdx) return false;
  ...
  if (leavingFinalState) m_view.resetEvalBar();
...
void HistorySystem::syncCapturedPieces() {
  ...
  for (std::size_t i = 0; i < m_fen_index && i < m_move_history.size(); ++i) {
    ...
    if (mv.move.isCapture()) m_view.addCapturedPiece(mv.moverColor, mv.capturedType);
  }
}

void HistorySystem::stashSelectedPiece() {
  m_stashed_selected = m_sel.getSelectedSquare();
  if (m_stashed_selected != core::NO_SQUARE) m_sel.deselectSquare();
}

void HistorySystem::restoreSelectedPiece() {
  if (m_stashed_selected != core::NO_SQUARE) m_sel.selectSquare(m_stashed_selected);
...
void HistorySystem::stepBackward(PremoveSystem& premove) {
  ...
  if (atHead()) stashSelectedPiece();

  if (m_fen_index == 0) return;

  const bool leavingFinalState = (m_game.getResult() != core::GameResult::ONGOING && atHead());

  if (atHead()) m_view.stashRightClickHighlights();
...
void HistorySystem::stepForward(PremoveSystem& premove) {
  if (m_fen_index >= m_move_history.size()) return;
  ...
  auto onMainMoveDone = [this, &premove]() { premove.restoreVisualsIfNeeded(atHead()); };
  ...
  if (atHead()) m_view.restoreRightClickHighlights();
}
