...
namespace lilia::app::view::ui
{
  ...
  void PieceManager::initFromFen(const std::string &fen)
  {
    ...
    for (char ch : boardPart)
    {
      ...
      else
      {
        ...
        addPiece(
            type,
            (std::isupper(static_cast<unsigned char>(ch)) ? chess::Color::White : chess::Color::Black),
            static_cast<chess::Square>(pos));
        file++;
      }
    }
  }
  ...
  /* -------------------- Query helpers -------------------- */
  [[nodiscard]] Entity::ID_type PieceManager::getPieceID(chess::Square pos) const
  {
    if (pos == chess::NO_SQUARE)
      return 0;
  ...
  [[nodiscard]] bool PieceManager::isSameColor(chess::Square sq1, chess::Square sq2) const
  {
    auto getPiece = [this](chess::Square sq) -> const PieceNode *
    {
    ...
    const PieceNode *p1 = getPiece(sq1);
    const PieceNode *p2 = getPiece(sq2);
    if (!p1 || !p2)
      return false;
  ...
  /* -------------------- Placement -------------------- */
  MousePos PieceManager::createPiecePositon(chess::Square pos)
  {
    return m_board_view_ref.getSquareScreenPos(pos) +
           MousePos{0.f, constant::SQUARE_PX_SIZE * 0.02f};
  }

  void PieceManager::addPiece(chess::PieceType type, chess::Color color, chess::Square pos)
  {
    PieceNode newpiece(color, type, getPieceTexture(type, color));
  ...
  void PieceManager::movePiece(chess::Square from, chess::Square to, chess::PieceType promotion)
  {
    PieceNode movingPiece;
    ...
    if (promotion != chess::PieceType::None)
    {
  ...
  void PieceManager::removePiece(chess::Square pos)
  {
    m_pieces.erase(pos);
  ...
  /* -------------------- Piece info -------------------- */
  chess::PieceType PieceManager::getPieceType(chess::Square pos) const
  {
    auto ghost = m_premove_pieces.find(pos);
    if (ghost != m_premove_pieces.end())
  ...
  chess::Color PieceManager::getPieceColor(chess::Square pos) const
  {
    auto ghost = m_premove_pieces.find(pos);
    if (ghost != m_premove_pieces.end())
  ...
  [[nodiscard]] bool PieceManager::hasPieceOnSquare(chess::Square pos) const
  {
  ...
  MousePos PieceManager::getPieceSize(chess::Square pos) const
  {
    auto ghost = m_premove_pieces.find(pos);
    if (ghost != m_premove_pieces.end())
  ...
  /* -------------------- Movement helpers -------------------- */
  [[nodiscard]] inline MousePos mouseToEntityPos(MousePos mousePos)
  {
    return static_cast<MousePos>(mousePos);
  }

  static auto findGhostByOrigin(std::unordered_map<chess::Square, chess::Square> &origin,
                                std::unordered_map<chess::Square, PieceNode> &ghosts, chess::Square from)
  ...
  void PieceManager::setPieceToSquareScreenPos(chess::Square from, chess::Square to)
  {
  ...
  void PieceManager::setPieceToScreenPos(chess::Square pos, MousePos mousePos)
  {
  ...
  void PieceManager::renderPiece(chess::Square pos, sf::RenderWindow &window)
  {
    if (m_hidden_squares.count(pos) > 0)
      return;
    auto it = m_pieces.find(pos);
  ...
  static PieceNode makeGhost(chess::PieceType type, chess::Color color)
  {
    PieceNode p(color, type, getPieceTexture(type, color));
    p.setScale(constant::PIECE_SCALE, constant::PIECE_SCALE);
    return p;
  }

  void PieceManager::setPremovePiece(chess::Square from, chess::Square to, chess::PieceType promotion)
  {
    PieceNode ghost;
    chess::Square origin = from;
    ...
    else
    {
      ...
      if (it == m_pieces.end())
        return;
      chess::PieceType gType = (promotion != chess::PieceType::None) ? promotion : it->second.getType();
  ...
  void PieceManager::consumePremoveGhost(chess::Square from, chess::Square to)
  {
  ...
  void PieceManager::applyPremoveInstant(chess::Square from, chess::Square to,
  ...
  void PieceManager::onPaletteChanged()
  {
    ...
    auto reload = [](PieceNode &p)
    {
      p.setTexture(getPieceTexture(p.getType(), p.getColor()));
      p.setScale(constant::PIECE_SCALE, constant::PIECE_SCALE);
    };
