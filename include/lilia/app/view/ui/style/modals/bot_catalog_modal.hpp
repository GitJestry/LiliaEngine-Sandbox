...
#include "lilia/app/view/ui/platform/file_dialog.hpp"
#include "lilia/app/view/ui/render/engine_icons.hpp"
...
{
  class BotCatalogModal final : public Modal
  {
  public:
    // current: engineId of currently selected engine (empty => none)
    BotCatalogModal(const sf::Font &font, const Theme &theme, std::string currentEngineId)
    ...
    std::optional<domain::analysis::config::EngineRef> picked() const { return m_picked; }
    ...
    void layout(sf::Vector2u ws) override
    {
      ...
      m_rect = anchoredCenter(ws, {w, h});
      m_inner = inset(m_rect, 18.f);

      sf::FloatRect top = rowConsume(m_inner, 44.f, 10.f);
      m_title.setPosition(snap({top.left, top.top + 8.f}));
      m_close.setBounds({top.left + top.width - 120.f, top.top + 6.f, 120.f, 32.f});

      sf::FloatRect hint = rowConsume(m_inner, 22.f, 6.f);
      m_hint.setPosition(snap({hint.left, hint.top}));

      sf::FloatRect status = rowConsume(m_inner, 22.f, 10.f);
      m_status.setPosition(snap({status.left, status.top}));

      sf::FloatRect sep = rowConsume(m_inner, 10.f, 10.f);
    ...
    {
      drawPanelShadow(win, m_rect);

      sf::RectangleShape panel({m_rect.width, m_rect.height});
      panel.setPosition(snap({m_rect.left, m_rect.top}));
      ...
      sep.setPosition(snap({m_sepRect.left, m_sepRect.top}));
      ...
      for (std::size_t i = 0; i < m_rows.size(); ++i)
      {
        ...
        drawBevelButton(win, r, base, hov, false);
  ...
  private:
    struct Row
    {
      domain::analysis::config::EngineRef ref;
      bool builtin{false};
    };
    ...
    void rebuildFromRegistry()
    {
      ...
      auto looksLike = [&](const engines::EngineEntry &e, const std::string &token)
      {
    ...
    void onUploadClicked()
    {
      ...
      auto path = platform::openExecutableFileDialog();
      if (!path || path->empty())
        return;
    ...
    std::optional<domain::analysis::config::EngineRef> m_picked{};
