...
namespace lilia::app::view::ui
{
  ...
  Entity PlayerInfoView::makeCapturedEntity(chess::PieceType type, chess::Color color)
  {
  ...
  void PlayerInfoView::setOutcome(std::optional<domain::Outcome> outcome)
  {
    // Normalize Unknown -> hide (keeps call sites simple)
    if (outcome && *outcome == domain::Outcome::Unknown)
      outcome.reset();
    ...
    switch (*m_outcome)
    {
    case domain::Outcome::Win:
      m_outcomeText.setString("WIN");
      break;
    case domain::Outcome::Loss:
      m_outcomeText.setString("LOSS");
      break;
    case domain::Outcome::Draw:
  ...
  void PlayerInfoView::setPlayerColor(chess::Color color)
  {
    m_playerColor = color;
  }

  void PlayerInfoView::setInfo(const domain::PlayerInfo &info)
  {
    ...
    m_icon.setPosition(snap({m_position.x + kIconFrameSize * 0.5f,
  ...
  void PlayerInfoView::setPosition(const MousePos &pos)
  {
    m_position = pos;
  ...
  void PlayerInfoView::setPositionClamped(const MousePos &pos, const sf::Vector2u &viewport)
  {
    const float outerW = kIconFrameSize;
    const float outerH = kIconFrameSize;

    constexpr float pad = 8.f;
    MousePos clamped = pos;
  ...
  void PlayerInfoView::layoutText()
  {
    ...
    m_name.setPosition(snap({textLeft, nameY}));

    if (!eloDisp.empty())
    {
      const float eloY = top + nb.height + gap - eb.top;
      m_elo.setPosition(snap({textLeft, eloY}));
    }
    else
    {
      m_elo.setPosition(snap({textLeft, nameY}));
    }
    ...
    if (hasOutcome)
    {
      ...
      const float pillX = snapf(nameRight + kOutcomeGap);
      const float pillY = snapf(nameY + (nb.height - pillH) * 0.5f);
      ...
      m_outcomeText.setPosition(snap({tx, ty}));
    }
    else
    {
      m_outcomePill.setSize({0.f, 0.f});
  ...
  void PlayerInfoView::render(sf::RenderWindow &rt)
  {
    ...
    const sf::Color capBase = (m_playerColor == chess::Color::White)
                                  ? pal[ColorId::COL_LIGHT_BG]
                                  : pal[ColorId::COL_DARK_BG];

    const sf::Color capText = (m_playerColor == chess::Color::White)
    ...
    drawSoftShadowRect(rt, fb, shadow, 1, 2.f);
    drawBevelButton(rt, fb, frameBase, false, false);

    m_icon.draw(rt);
    if (m_outcome && m_outcomePill.getSize().x > 0.f)
    {
      const Theme *th = m_theme;
      ...
      if (th)
      {
        ...
        case domain::Outcome::Win:
          fill = th->accent;
          text = th->onAccent;
          outline = darken(th->accent, 18);
          break;
        case domain::Outcome::Loss:
          fill = darken(th->panel, 8);
          text = th->subtle;
          outline = th->panelBorder;
          break;
        case domain::Outcome::Draw:
    ...
    drawSoftShadowRect(rt, cb, shadow, 1, 2.f);
    drawBevelButton(rt, cb, capBase, false, false);
  ...
  void PlayerInfoView::addCapturedPiece(chess::PieceType type, chess::Color color)
  {
