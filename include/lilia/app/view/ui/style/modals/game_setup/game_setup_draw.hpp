...
namespace lilia::app::view::ui::game_setup
{
  ...
  static inline void draw_label(sf::RenderTarget &rt, const sf::Font &font, const Theme &theme,
                                float x, float y, const std::string &txt, unsigned size = 16)
  {
    sf::Text t(txt, font, size);
    t.setFillColor(theme.subtle);
    t.setPosition(snap({x, y}));
    rt.draw(t);
  }
  ...
  // - err: red
  // - neutral: subdued
  static inline void draw_status_pill(sf::RenderTarget &rt, const sf::Font &font, const Theme &theme,
                                      const sf::FloatRect &r, const std::string &txt, int kind)
  {
    ...
    box.setPosition(snap({r.left, r.top}));
    ...
    t.setPosition(snap({r.left + 4.f, r.top + (r.height - b.height) * 0.5f - b.top}));
    rt.draw(t);
  }

  static inline void draw_section_card(sf::RenderTarget &rt, const Theme &theme, const sf::FloatRect &r)
  {
