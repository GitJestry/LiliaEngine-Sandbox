...
namespace lilia::app::domain::notation
{
  ...
  bool parsePgnToRecord(std::string_view pgn, GameRecord &out, std::string *err)
  {
    ...
    if (out.startFen.empty())
      out.startFen = std::string{chess::constant::START_FEN};
    ...
    for (const std::string &t : toks)
    {
      ...
      chess::Move mv;
      const chess::Position &pos = g.getPositionRefForBot();
      ...
      PlyRecord pr{};
      pr.move = mv;
      pr.timeAfter = analysis::TimeView{}; // optional; keep zero unless you parse %clk
