...
namespace lilia::app::view::ui
{
  ...
  void EvalBar::setPosition(const MousePos &pos)
  {
  ...
  void EvalBar::render(sf::RenderWindow &window)
  {
    ...
    // --- Toggle button (always visible) ---
    {
      ...
      drawSoftShadowRect(window, m_toggle_bounds, pal[ColorId::COL_SHADOW_LIGHT], 1, 2.f);

      sf::Color top = m_visible ? lighten(pal[ColorId::COL_ACCENT], 30)
                                : lighten(pal[ColorId::COL_HEADER], 10);
      sf::Color bot = m_visible ? darken(pal[ColorId::COL_ACCENT], 25)
                                : darken(pal[ColorId::COL_HEADER], 12);
      ...
      drawBevelFrame(window, m_toggle_bounds, bevelBase, pal[ColorId::COL_BORDER_BEVEL]);
    ...
    drawSoftShadowRect(window, barRect, pal[ColorId::COL_SHADOW_LIGHT], 1, 2.f);
    ...
    drawBevelFrame(window, barRect, pal[ColorId::COL_HEADER], pal[ColorId::COL_BORDER_BEVEL]);
  ...
  void EvalBar::scaleToEval(float e)
  {
    ...
    m_white_fill_eval.setPosition(MousePos{p.x, m_flipped ? p.y - off : p.y + off});
  ...
  bool EvalBar::isOnToggle(MousePos mousePos) const
  {
