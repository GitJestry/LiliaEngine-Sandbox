...
#include "lilia/app/view/ui/style/color_palette_manager.hpp"
...
#include "lilia/app/view/ui/widgets/time_control_picker.hpp"

#include "lilia/app/view/ui/style/modals/game_setup/game_setup_modal.hpp"
#include "lilia/app/view/ui/style/modals/game_setup/game_setup_validation.hpp"
#include "lilia/app/view/ui/style/modals/palette_picker_modal.hpp"
...
#include <commdlg.h>
    ...
    static std::string engineLabel(const domain::analysis::config::EngineRef &r)
    {
  ...
  domain::analysis::config::StartConfig StartScreen::run()
  {
    ModalStack modals;
    FocusManager focus;
    ...
    auto &reg = engines::EngineRegistry::instance();

    // Canonical StartConfig (single source of truth).
    domain::analysis::config::StartConfig cfg{};
    cfg.game.startFen = std::string{chess::constant::START_FEN};
    ...
    cfg.white.kind = domain::analysis::config::SideKind::Human;
    cfg.white.bot.reset();

    auto makeLockedInBot = [&]() -> domain::analysis::config::BotConfig
    {
    ...
    {
      cfg.black.kind = domain::analysis::config::SideKind::Engine;
    }
    else
    {
      // Safe degrade: never claim "bot" if we don't have an engine.
      cfg.black.kind = domain::analysis::config::SideKind::Human;
      cfg.black.bot.reset();
    }

    // Invariant helpers (prevents "engine side but nobody plays" + fixes icon fallback issues).
    auto setHumanSide = [&](domain::analysis::config::SideConfig &side)
    {
      side.kind = domain::analysis::config::SideKind::Human;
      side.bot.reset();
    };

    auto ensureBotSide = [&](domain::analysis::config::SideConfig &side)
    {
      side.kind = domain::analysis::config::SideKind::Engine;
    ...
    auto applyPickedEngine = [&](domain::analysis::config::SideConfig &side, const domain::analysis::config::EngineRef &picked)
    {
      // Preserve existing limits, but reset UCI values to the new engine's defaults.
      domain::analysis::config::SearchLimits limits{};
      if (side.bot)
    ...
    TimeControlPicker timePicker(m_font, theme);
    ...
    // Track modals
    PalettePickerModal *paletteModal = nullptr;
    BotCatalogModal *whiteCatalog = nullptr;
    BotCatalogModal *blackCatalog = nullptr;
    game_setup::GameSetupModal *setupModal = nullptr;
    ...
    paletteBtn.setOnClick([&]
                          {
      auto m = std::make_unique<PalettePickerModal>();
      paletteModal = m.get();

      PalettePickerModal::Params p;
      p.theme = &theme;
      p.font = &m_font;
    ...
    loadBtn.setOnClick([&]
                       {
      auto m = std::make_unique<game_setup::GameSetupModal>(m_font, theme, focus);
      setupModal = m.get();
      ...
      setupModal->setOnRequestPgnUpload([&, modalPtr = setupModal]
      {
        ...
        auto imported = game_setup::import_pgn_file(*path);
        if (!imported)
          return;
    ...
    auto layoutAll = [&]
    {
      ...
      panel = anchoredCenter(ws, {panelW, panelH});
      sf::FloatRect inner = inset(panel, 24.f);
      ...
      titlePos = snap({panel.left + 24.f, panel.top + 18.f});
      subtitlePos = snap({panel.left + 24.f, panel.top + 52.f});
      ...
      whiteLabelPos = snap({whiteCol.left, whiteCol.top});
      blackLabelPos = snap({blackCol.left, blackCol.top});
    ...
    while (m_window.isOpen() && !done)
    {
      ...
      while (m_window.pollEvent(e))
      {
        ...
        const bool whiteIsBot =
            (cfg.white.kind == domain::analysis::config::SideKind::Engine) &&
      ...
      modals.update(dt, mouse, [&](Modal &modal)
                    {
        ...
        if (whiteCatalog && &modal == whiteCatalog)
        {
          ...
          if (cfg.white.kind == domain::analysis::config::SideKind::Engine)
            ensureBotSide(cfg.white);
        ...
        if (blackCatalog && &modal == blackCatalog)
        {
          ...
          // Ensure invariants even if modal closed without a selection.
          if (cfg.black.kind == domain::analysis::config::SideKind::Engine)
            ensureBotSide(cfg.black);
      ...
      {
        drawSoftShadowRect(m_window, r, sf::Color(0, 0, 0, 70), 2, 2.f);

        sf::RectangleShape card({r.width, r.height});
        card.setPosition(snap({r.left, r.top}));
      ...
      const bool whiteIsBot = (cfg.white.kind == domain::analysis::config::SideKind::Engine) && cfg.white.bot.has_value();
      const bool blackIsBot = (cfg.black.kind == domain::analysis::config::SideKind::Engine) && cfg.black.bot.has_value();
      ...
      {
        sf::Text ver(std::string("SandBox 1.0v"), m_font, 14);
        ver.setFillColor(theme.subtle);
        auto vb = ver.getLocalBounds();
        auto ws = m_window.getSize();
        ver.setPosition(snap({(ws.x - vb.width) * 0.5f, float(ws.y) - 26.f}));
        m_window.draw(ver);
      }

      {
        ...
        auto ws = m_window.getSize();
        credit.setPosition(snap({float(ws.x) - cb.width - 18.f, float(ws.y) - cb.height - 22.f}));
