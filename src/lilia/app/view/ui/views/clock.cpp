...
namespace lilia::app::view::ui
{
  ...
  void Clock::updateVisualState()
  {
    ...
    const bool isLight = (m_playerColor == chess::Color::White);
    ...
    if (m_active)
    {
      tweakedFill = isLight ? darken(baseFill, 18) : lighten(baseFill, 16);
    }
    ...
    if (m_active)
    {
      ...
      m_box.setOutlineColor(lerpColor(pal[ColorId::COL_BORDER], pal[ColorId::COL_CLOCK_ACCENT], 0.65f));
    ...
    const sf::Color iconCol = lerpColor(pal[ColorId::COL_CLOCK_ACCENT], baseText, mixT);
  ...
  void Clock::setPlayerColor(chess::Color color)
  {
    m_playerColor = color;
    updateVisualState();
  }

  void Clock::setPosition(const MousePos &pos)
  {
