...
namespace lilia::app::view::ui
{
  ...
  public:
    ...
    void draw(sf::RenderTarget &rt, MousePos offset = {}) const
    {
      ...
      if (focused() && !m_readOnly)
      {
        ...
        if (blink < 0.5f)
        {
          const MousePos cp = caretPosPx_(gb);
  ...
  private:
    ...
    MousePos caretPosPx_(const sf::FloatRect &gb) const
    {
      ensureLayout_();
      const float pad = 8.f;
      const float x0 = gb.left + pad;
    ...
    size_t caretFromMouse_(const sf::FloatRect &gb, MousePos mouse) const
    {
