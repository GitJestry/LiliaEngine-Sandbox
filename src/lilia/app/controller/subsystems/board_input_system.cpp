...
namespace lilia::app::controller
{
  ...
  BoardInputSystem::BoardInputSystem(view::ui::GameView &view, chess::ChessGame &game,
                                     InputManager &input, SelectionManager &sel,
                                     view::audio::SoundManager &sfx, PremoveSystem &premove, AttackSystem &att)
      : m_view(view),
        m_game(game),
        m_input(input),
        m_sel(sel),
  ...
  void BoardInputSystem::onMouseMove(MousePos pos)
  {
    ...
    const chess::Square sq = m_view.mousePosToSquare(pos);
  ...
  void BoardInputSystem::onMousePressed(MousePos pos)
  {
    ...
    const chess::Square currentSelected = m_sel.getSelectedSquare();
    const bool selectionWasDifferent = (currentSelected != sq);

    if (currentSelected != chess::NO_SQUARE && currentSelected != sq)
    {
    ...
    else
    {
      ...
      m_prev_selected_before_preview = chess::NO_SQUARE;
  ...
  void BoardInputSystem::onRightReleased(MousePos pos)
  {
    ...
    const chess::Square endSq = m_view.mousePosToSquare(pos);
    const chess::Square startSq = m_right_drag_from;
    m_right_drag_from = chess::NO_SQUARE;

    if (!chess::validSquare(startSq) || !chess::validSquare(endSq))
      return;
  ...
  bool BoardInputSystem::isHumanPiece(chess::Square sq) const
  {
  ...
  bool BoardInputSystem::tryMove(chess::Square a, chess::Square b) const
  {
    if (!isHumanPiece(a))
      return false;
  ...
  void BoardInputSystem::showAttacks(const std::vector<chess::Square> &att)
  {
    m_view.clearAttackHighlights();
    for (auto sq : att)
    {
  ...
  void BoardInputSystem::onClick(MousePos mousePos)
  {
    ...
    const chess::Square sq = m_view.mousePosToSquare(mousePos);
    if (!chess::validSquare(sq))
    {
      m_selection_changed_on_press = false;
    ...
    if (m_view.isInPromotionSelection())
    {
      const chess::PieceType promoType = m_view.getSelectedPromotion(mousePos);
    ...
    if (m_sel.getSelectedSquare() != chess::NO_SQUARE)
    {
      ...
      const bool ownTurnAndPiece =
          (selPiece.type != chess::PieceType::None && st.sideToMove == selPiece.color &&
           (!m_game_manager || m_game_manager->isHuman(st.sideToMove)));

      const chess::Color humanColor = ~st.sideToMove;
      const bool canPremove =
          (selPiece.type != chess::PieceType::None && selPiece.color == humanColor &&
  ...
  void BoardInputSystem::onDrop(MousePos start, MousePos end)
  {
    ...
    const auto st = m_game.getGameState();
    const chess::Color fromColor = m_premove.pieceConsideringPremoves(from).color;
    ...
    const chess::Color humanNextColor = ~st.sideToMove;
    ...
    if (!accepted)
    {
      if (!setPremove)
      {
        ...
        m_view.clearNonPremoveHighlights();
        m_sel.highlightLastMove();
        m_sel.selectSquare(m_preview_active && chess::validSquare(m_prev_selected_before_preview) &&
  ...
  {
    chess::Square activeSq = chess::NO_SQUARE;
    if (m_dragging && chess::validSquare(m_drag_from))
      activeSq = m_drag_from;
    else if (chess::validSquare(m_sel.getSelectedSquare()))
      activeSq = m_sel.getSelectedSquare();

    if (chess::validSquare(activeSq) && isHumanPiece(activeSq))
