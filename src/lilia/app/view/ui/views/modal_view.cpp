...
namespace lilia::app::view::ui
{
  ...
  void ModalView::showResign(sf::Vector2u ws, sf::Vector2f /*anchorCenter*/)
  {
    ...
    auto modal = std::make_unique<ConfirmResignModal>();
    m_resign = modal.get();

    ConfirmResignModal::Params p;
    p.theme = &m_theme.uiTheme();
    p.font = m_fontLoaded ? &m_font : nullptr;

    p.onYes = [this]()
    { m_pending = ModalAction::ResignYes; };
    p.onNo = [this]()
    { m_pending = ModalAction::ResignNo; };
    p.onClose = [this]()
    { m_pending = ModalAction::Close; };
  ...
  void ModalView::showGameOver(sf::Vector2u ws, const std::string &msg, bool won, sf::Vector2f anchorCenter)
  {
    ...
    if (m_gameOver && m_gameOver->isOpen())
    {
      GameOverModal::Params p;
      p.theme = &m_theme.uiTheme();
      p.font = m_fontLoaded ? &m_font : nullptr;

      p.onNewBot = [this]()
      { m_pending = ModalAction::NewBot; };
      p.onRematch = [this]()
      { m_pending = ModalAction::Rematch; };
      p.onClose = [this]()
      { m_pending = ModalAction::Close; };
    ...
    auto modal = std::make_unique<GameOverModal>();
  ...
  bool ModalView::handleEvent(const sf::Event &e, MousePos mouse)
  {
    if (m_stack.empty())
      return false;
  ...
  void ModalView::update(float dt, MousePos mouse)
  {
    m_stack.update(dt, mouse, [this](Modal &m)
                   { trackDismissed(m); });
  }
  ...
  ModalAction ModalView::consumeAction()
  {
    if (!m_pending)
      return ModalAction::None;

    const ModalAction out = *m_pending;
    m_pending.reset();
    return out;
  }

  void ModalView::trackDismissed(Modal &m)
  {
    if (&m == m_resign)
      m_resign = nullptr;
    if (&m == m_gameOver)
