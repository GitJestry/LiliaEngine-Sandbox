#include "lilia/app/view/ui/style/modals/game_setup/game_setup_page_pgn_fen.hpp"
...
{
  PagePgnFen::PagePgnFen(const sf::Font &font, const Theme &theme, FocusManager &focus)
      : m_font(font), m_theme(theme), m_focus(focus)
  {
    ...
    m_fenField.setText(std::string{chess::constant::START_FEN});
  ...
  void PagePgnFen::layout(const sf::FloatRect &bounds)
  {
    ...
    // --- FEN card ---
    {
      sf::FloatRect inner = inset(m_fenCard, 12.f);

      m_fenHeader = rowConsume(inner, 18.f, 8.f);

      sf::FloatRect row = rowConsume(inner, 36.f, 8.f);
      ...
      m_fenStatusLine = rowConsume(inner, 18.f, 0.f);
    }

    // --- PGN card ---
    {
      sf::FloatRect inner = inset(m_pgnCard, 12.f);

      m_pgnHeader = rowConsume(inner, 18.f, 8.f);
    ...
    // --- Resolved card ---
    {
      sf::FloatRect inner = inset(m_resolvedCard, 12.f);

      m_resolvedHeader = rowConsume(inner, 18.f, 10.f);
      ...
      sf::FloatRect row = rowConsume(inner, rowH, 0.f);
  ...
  void PagePgnFen::updateHover(MousePos mouse)
  {
  ...
  bool PagePgnFen::handleEvent(const sf::Event &e, MousePos mouse)
  {
  ...
  void PagePgnFen::draw(sf::RenderTarget &rt) const
  {
    ...
    if (!m_pgnFilename.empty())
    {
      ...
      fn.setPosition(snap({m_pgnHeader.left + 52.f, m_pgnHeader.top}));
      rt.draw(fn);
    }
  ...
  void PagePgnFen::setup_action(Button &b, const char *txt, std::function<void()> cb)
  {
    b.setTheme(&m_theme);
    b.setFont(m_font);
    b.setText(txt, 12);
  ...
  void PagePgnFen::setup_chip(Button &b, const char *txt, Source s)
  {
    b.setTheme(&m_theme);
    b.setFont(m_font);
    b.setText(txt, 16);
  ...
  std::string PagePgnFen::compute_resolved_fen() const
  {
    ...
    {
    case Source::Fen:
      return fenOk ? m_fenSanitized : std::string{chess::constant::START_FEN};
    ...
    case Source::Pgn:
      ...
      if (m_pgnStatus.kind == PgnStatus::Kind::OkNoFen)
        return std::string{chess::constant::START_FEN}; // replay still starts from standard

      return std::string{chess::constant::START_FEN};
