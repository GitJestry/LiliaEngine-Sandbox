...
#include "lilia/app/view/ui/style/modals/game_setup/position_builder.hpp"
#include "lilia/app/view/ui/style/modals/game_setup/position_builder_rules.hpp"
...
namespace lilia::app::view::ui::game_setup
{
  ...
  struct PositionBuilder::Impl
  {
    ...
    void hookCallbacks()
    {
      ...
      btnCopyFen.setOnClick([&]
                            {
        if (!rules::kingsOk(board) || !rules::pawnsOk(board))
        {
    ...
    void restoreHeldAfterEpCancelOrCommit()
    {
      ...
      if (epHold->dragging && epHold->dragPiece != '.' && epHold->dragFrom)
      {
        ...
        auto [ox, oy] = *dragFrom;
        if (rules::inBounds(ox, oy) && board[oy][ox] == dragPiece)
          board[oy][ox] = '.';
      }
      else
      {
    ...
    void refreshKings()
    {
      rules::countKings(board, whiteK, blackK);
    }

    void sanitizeMeta()
    {
      rules::sanitizeMeta(board, meta);
    }
    ...
    {
      return rules::fen(board, meta);
    }

    std::string fenForUse() const
    {
      if (!rules::kingsOk(board))
        return {};
      if (!rules::pawnsOk(board))
        return {};
      return fen();
    }
    ...
    static SetFailReason mapFail(rules::PlacementFailReason r)
    {
      switch (r)
      {
      case rules::PlacementFailReason::PawnOnLastRank:
        return SetFailReason::PawnOnLastRank;
      case rules::PlacementFailReason::KingUniqueness:
        return SetFailReason::KingUniqueness;
      case rules::PlacementFailReason::None:
      default:
    ...
    bool trySet(int x, int y, char p, bool remember)
    {
      ...
      const rules::PlacementFailReason v = rules::validateSetPiece(board, x, y, p);
      if (v != rules::PlacementFailReason::None)
      {
    ...
    std::optional<std::pair<int, int>> squareFromMouse(MousePos local) const
    {
    ...
    void toggleCastle(bool white, bool kingSide)
    {
      ...
      if (!rules::hasCastleStructure(board, white, kingSide))
      {
    ...
    {
      clear(false);
      setFromFen(std::string{chess::constant::START_FEN}, false);
    ...
    {
      rules::setFromFen(board, meta, fenStr);
    ...
    // ---------- input ----------
    void updateHover(MousePos mouse, MousePos off)
    {
      mouseGlobal = mouse;
      offset = off;

      const MousePos local{mouse.x - off.x, mouse.y - off.y};
    ...
    std::optional<int> paletteIndexAt(MousePos local) const
    {
      for (int i = 0; i < 12; ++i)
    ...
    bool handlePaletteInteraction(const sf::Event &e, MousePos mouse, MousePos off)
    {
      const MousePos local{mouse.x - off.x, mouse.y - off.y};
      ...
      if (e.type == sf::Event::MouseMoved && palettePress && paletteIdx >= 0 && paletteIdx < 12)
      {
        ...
        const MousePos d = local - palettePressLocal;
    ...
    bool handleEvent(const sf::Event &e, MousePos mouse, MousePos off)
    {
      if (!theme || !font)
        return false;
      ...
      if (epSelecting)
      {
            ...
            (e.mouseButton.button == sf::Mouse::Left || e.mouseButton.button == sf::Mouse::Right))
        {
          ...
          {
            auto [x, y] = *sqr;
            if (rules::isValidEnPassantTarget(board, x, y, meta.sideToMove))
            {
    ...
    // ---------- draw ----------
    void draw(sf::RenderTarget &rt, MousePos off) const
    {
      if (!theme || !font)
        return;
    ...
    void drawPanel(sf::RenderTarget &rt, MousePos off, const sf::FloatRect &r, const char *title) const
    {
    ...
    void drawToolSegmentedControl(sf::RenderTarget &rt, MousePos off) const
    {
      // Background track
    ...
    void drawHotkeysField(sf::RenderTarget &rt, MousePos off) const
    {
    ...
    void drawSidePanels(sf::RenderTarget &rt, MousePos off, MousePos shake) const
    {
      (void)shake;
    ...
    void drawFenPanel(sf::RenderTarget &rt, MousePos off) const
    {
      ...
      btnCastleK.setEnabled(meta.castleK || rules::hasCastleStructure(board, true, true));
      btnCastleQ.setEnabled(meta.castleQ || rules::hasCastleStructure(board, true, false));
      btnCastlek.setEnabled(meta.castlek || rules::hasCastleStructure(board, false, true));
      btnCastleq.setEnabled(meta.castleq || rules::hasCastleStructure(board, false, false));
      ...
      const bool ok = rules::kingsOk(board) && rules::pawnsOk(board);
    ...
    void drawBoard(sf::RenderTarget &rt, MousePos off, MousePos shake) const
    {
      ...
      if (hoverSquare)
      {
        ...
        const MousePos sqPos = ui::snap({boardRect.left + off.x + shake.x + hx * sq,
        ...
        if (!dragging && selected.kind == ToolKind::Add)
        {
          ...
          {
            const bool illegal =
                rules::validateSetPiece(board, hx, hy, selected.piece) != rules::PlacementFailReason::None;

            const MousePos center = ui::snap({sqPos.x + sq * 0.5f,
            ...
            auto drawOutlinedGhost = [&](sf::Color outline, sf::Color fill)
            {
              ...
              static const std::array<MousePos, 4> offs{
                  MousePos{-1.f, 0.f},
                  MousePos{1.f, 0.f},
                  MousePos{0.f, -1.f},
                  MousePos{0.f, 1.f},
    ...
    void drawEpSelectionOverlay(sf::RenderTarget &rt, MousePos off, MousePos shake) const
    {
    ...
    void drawPiece(sf::RenderTarget &rt, MousePos off, MousePos shake,
    ...
    void drawToast(sf::RenderTarget &rt, MousePos off) const
    {
    ...
    void drawError(sf::RenderTarget &rt, MousePos off) const
    {
  ...
  bool PositionBuilder::kingsOk() const { return rules::kingsOk(m->board); }
  int PositionBuilder::whiteKings() const { return m->whiteK; }
  int PositionBuilder::blackKings() const { return m->blackK; }

  void PositionBuilder::updateHover(MousePos mouse, MousePos off) { m->updateHover(mouse, off); }

  bool PositionBuilder::handleEvent(const sf::Event &e, MousePos mouse, MousePos off)
  {
    return m->handleEvent(e, mouse, off);
  }

  void PositionBuilder::draw(sf::RenderTarget &rt, MousePos off) const
  {
    m->draw(rt, off);
  }
