...
{
  GameSetupModal::GameSetupModal(const sf::Font &font, const Theme &theme, FocusManager &focus)
  ...
  void GameSetupModal::layout(sf::Vector2u ws)
  {
    ...
    m_rect = anchoredCenter(ws, {940.f, 680.f});
    m_inner = inset(m_rect, 18.f);

    sf::FloatRect header = rowConsume(m_inner, 44.f, 12.f);
    m_title.setPosition(snap({header.left, header.top + 9.f}));
    ...
    sf::FloatRect tabs = rowConsume(content, 32.f, 12.f);

    sf::FloatRect t = tabs;
    auto a = colConsume(t, 120.f, 8.f);
    auto b = colConsume(t, 92.f, 8.f);
  ...
  void GameSetupModal::updateInput(MousePos mouse, bool /*mouseDown*/)
  {
    m_mouse = mouse;
  ...
  void GameSetupModal::drawPanel(sf::RenderWindow &win) const
  {
    ...
    if (!m_showHistory)
    {
      ...
      glow.setPosition(snap({b.left - 5.f, b.top - 5.f}));
      glow.setFillColor(withA(m_theme.accent, 40));
      win.draw(glow);
  ...
  bool GameSetupModal::handleEvent(const sf::Event &e, MousePos mouse)
  {
    m_mouse = mouse;
  ...
  void GameSetupModal::setup_action(Button &b, const char *txt, std::function<void()> cb)
  {
    b.setTheme(&m_theme);
    b.setFont(m_font);
    b.setText(txt, 14);
