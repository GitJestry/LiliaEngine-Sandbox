...
namespace lilia::app::view::ui
{
  ...
  namespace
  {
    ...
    static unsigned int arrowKey(chess::Square from, chess::Square to)
    {
  ...
  void HighlightManager::renderEntitiesToBoard(std::unordered_map<chess::Square, Entity> &map,
  ...
  void HighlightManager::renderRightClickArrows(sf::RenderWindow &window)
  {
    ...
    {
      chess::Square fromSq = pr.first;
      chess::Square toSq = pr.second;
      if (fromSq == toSq)
        continue;
      ...
      if (knight)
      {
        ...
        chess::Square cornerSq =
            static_cast<chess::Square>(cornerFile + cornerRank * constant::BOARD_SIZE);
  ...
  void HighlightManager::highlightSquare(chess::Square pos)
  {
    m_select[pos] = makeHL(constant::tex::SELECT_HL, /*scaleToSquare=*/true);
  }

  void HighlightManager::highlightAttackSquare(chess::Square pos)
  {
    AttackMark m;
  ...
  void HighlightManager::highlightCaptureSquare(chess::Square pos)
  {
    AttackMark m;
  ...
  void HighlightManager::highlightHoverSquare(chess::Square pos)
  {
    m_hover[pos] = makeHL(constant::tex::HOVER_HL, /*scaleToSquare=*/false);
  }

  void HighlightManager::highlightPremoveSquare(chess::Square pos)
  {
    m_premove[pos] = makeHL(constant::tex::PREMOVE_HL, /*scaleToSquare=*/true);
  }

  void HighlightManager::highlightRightClickSquare(chess::Square pos)
  {
  ...
  void HighlightManager::highlightRightClickArrow(chess::Square from, chess::Square to)
  {
  ...
  std::vector<chess::Square> HighlightManager::getRightClickSquares() const
  {
  ...
  std::vector<std::pair<chess::Square, chess::Square>> HighlightManager::getRightClickArrows() const
  {
    std::vector<std::pair<chess::Square, chess::Square>> out;
  ...
  void HighlightManager::clearHighlightSquare(chess::Square pos) { m_select.erase(pos); }
  void HighlightManager::clearHighlightHoverSquare(chess::Square pos) { m_hover.erase(pos); }
  void HighlightManager::clearHighlightPremoveSquare(chess::Square pos) { m_premove.erase(pos); }
