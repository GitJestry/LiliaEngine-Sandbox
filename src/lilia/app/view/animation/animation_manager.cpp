#include "lilia/app/view/animation/animation_manager.hpp"
...
namespace lilia::app::view::animation
{
  ...
  void AnimationManager::add(ui::Entity::ID_type entityID, std::unique_ptr<IAnimation> anim)
  {
    addOrReplace(entityID, std::move(anim), AnimLayer::Base);
  }

  void AnimationManager::addOrReplace(ui::Entity::ID_type entityID, std::unique_ptr<IAnimation> anim,
                                      AnimLayer layer)
  {
  ...
  [[nodiscard]] bool AnimationManager::isAnimating(ui::Entity::ID_type entityID) const
  {
    return m_animations.find(entityID) != m_animations.end();
  }

  [[nodiscard]] bool AnimationManager::hasInAnyLayer(ui::Entity::ID_type entityID) const
  {
    return m_animations.find(entityID) != m_animations.end() ||
           m_highlight_level_animations.find(entityID) != m_highlight_level_animations.end();
  }

  void AnimationManager::declareHighlightLevel(ui::Entity::ID_type entityID)
  {
  ...
  void AnimationManager::endAnim(ui::Entity::ID_type entityID)
  {
    m_animations.erase(entityID);
  }

  void AnimationManager::cancelAll(ui::Entity::ID_type entityID)
  {
    if (auto it = m_animations.find(entityID); it != m_animations.end())
    {
      m_animations.erase(it);
