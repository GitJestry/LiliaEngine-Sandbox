...
namespace lilia::app::view::ui
{
  ...
  static std::string iconForEngine(const domain::analysis::config::EngineRef &ref)
  {
  ...
  {
    domain::PlayerInfo info{};
    info.elo = "";

    if (side.kind == domain::analysis::config::SideKind::Human || !side.bot.has_value() ||
