...
namespace lilia::app::view::ui
{
  ...
  void Entity::setPosition(const MousePos &pos)
  {
    m_sprite.setPosition(pos);
  }

  [[nodiscard]] MousePos Entity::getPosition() const
  {
  ...
  {
    MousePos bounds = getOriginalSize();
    m_sprite.setOrigin(bounds.x / 2.f, bounds.y / 2.f);
  }

  void Entity::setOrigin(MousePos org)
  {
  ...
  Entity::Entity(MousePos pos) : Entity()
  {
    m_sprite.setPosition(pos);
  }
  ...
  Entity::Entity(const sf::Texture &texture, MousePos pos) : m_id(generateId()), m_sprite(texture)
  {
    setOriginToCenter();
    m_sprite.setPosition(pos);
  }
  [[nodiscard]] MousePos Entity::getOriginalSize() const
  {
    return MousePos(m_sprite.getLocalBounds().width, m_sprite.getLocalBounds().height);
  }
  [[nodiscard]] MousePos Entity::getCurrentSize() const
  {
    return MousePos(m_sprite.getGlobalBounds().width, m_sprite.getGlobalBounds().height);
  }
