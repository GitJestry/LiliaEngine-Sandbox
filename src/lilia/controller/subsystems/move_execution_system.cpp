...
void MoveExecutionSystem::applyMove(const model::Move& move, bool isPlayerMove, bool onClick) {
  ...
  if (move.castle() != model::CastleSide::None) {
    ...
    const core::Square rookTo = (move.castle() == model::CastleSide::KingSide)
                                    ...
                                    : static_cast<core::Square>(to + 1);
    if (!skipAnim) m_view.animationMovePiece(rookFrom, rookTo);
  }
  ...
  if (move.isCapture()) m_view.addCapturedPiece(moverColorBefore, capturedType);

  m_view.addMove(move_to_uci(move));
  ...
  const TimeView tv =
      m_clock.enabled() ? m_clock.snapshot(sideToMoveNow) : TimeView{0.f, 0.f, sideToMoveNow};

  MoveView mvInfo{move, moverColorBefore, capturedType, effect, m_eval_cp.load()};
  m_history.onMoveCommitted(mvInfo, m_game.getFen(), m_eval_cp.load(), tv);
