...
namespace lilia::app::view::ui
{
  ...
  MousePos BoardView::getSquareScreenPos(chess::Square sq) const
  {
    if (m_flipped)
    {
      return m_board.getPosOfSquare(
          static_cast<chess::Square>(constant::BOARD_SIZE * constant::BOARD_SIZE - 1 - sq));
    }
  ...
  void BoardView::setPosition(const MousePos &pos)
  {
  ...
  MousePos BoardView::getPosition() const { return m_board.getPosition(); }

  bool BoardView::isOnFlipIcon(MousePos mousePos) const
  {
  ...
  MousePos BoardView::clampPosToBoard(MousePos mousePos,
                                      MousePos pieceSize) const noexcept
  {
  ...
  chess::Square BoardView::mousePosToSquare(MousePos mousePos) const
  {
    auto boardCenter = getPosition();
