...
namespace lilia::model
{
  ...
  bool Position::checkInsufficientMaterial()
  {
    ...
    const bb::Bitboard nonMinors =
        m_board.getPieces(core::Color::White, core::PieceType::Pawn) |
    ...
    auto is_dark = [](core::Square s) -> bool
    {
      ...
      if (!isCap)
      {
        if (white)
        {
          if (df != 8 && !(df == 16 && fromRank == 1))
            return false;
          if (occ & bb::sq_bb(static_cast<core::Square>(m.from() + 8)))
            return false;
  ...
  bool Position::see(const model::Move &m) const
  {
    ...
    { return engine::base_value[(int)pt]; };
  ...
  bool Position::doMove(const Move &m)
  {
    ...
    {
      if (fromPiece->type != core::PieceType::Pawn)
        return false;
  ...
  void Position::unapplyMove(const StateInfo &st)
  {
    ...
    m_state.sideToMove = ~m_state.sideToMove;
    hashXorSide();
    if (m_state.sideToMove == core::Color::Black)
      --m_state.fullmoveNumber;
